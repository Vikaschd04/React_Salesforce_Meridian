...
| Sales tax | flat rate (`SF_TAX_RATE`, default 8%) on the post-discount subtotal, computed server-side (`lib/totals.computeTax`), included in the charge, and stored as a standard **`OrderItemTaxLineItem`** (one per product line, apportioned by line amount) → rolls up to `OrderSummary.TotalTaxAmount`. No custom tax field. |
...
2. One **`OrderItemTaxLineItem`** *per product line* carries that line's share of
   the sales tax (apportioned by line amount, cents summing exactly to the total),
   so each `OrderItemSummary` reflects its own tax.
